(() => {
  ...
  function splitTenderButtons(text) {
    ...
    const positions = titles.map((t) => {
      const re = new RegExp(`^${t}\\s*$`, "m");
      const m = re.exec(text);
      return m ? { title: t, start: m.index, headerEnd: m.index + m[0].length } : null;
    });
    if (positions.some((p) => p === null)) {
      throw new Error("splitTenderButtons: missing section header");
    }
    const books = [];
    for (let i = 0; i < positions.length; i++) {
      const title = positions[i].title;
      const bodyStart = positions[i].headerEnd;
      const bodyEnd = i + 1 < positions.length ? positions[i + 1].start : text.length;
      const inner = text.slice(bodyStart, bodyEnd).trim();
...
${inner}` });
  ...
  function minWidthForLineCount(prepared, targetLines, maxWidth, lineHeight) {
    if (prepared.widths.length === 0) return 1;
    if (layout(prepared, maxWidth, lineHeight).lineCount > targetLines) return maxWidth;
    let lo = 1;
    let hi = Math.max(1, Math.floor(maxWidth));
    while (lo < hi) {
      const mid = lo + hi >> 1;
      if (layout(prepared, mid, lineHeight).lineCount > targetLines) lo = mid + 1;
      else hi = mid;
    }
    return lo;
  }
  function buildColumnsGrid(books, ctx2, fontFamily, lineHeightRatio, viewportWidth, viewportHeight, opts = {}) {
    const gutter = Math.max(0, opts.gutter ?? 16);
    const minLines = opts.minLines ?? 4;
    const numGutters = Math.max(0, books.length - 1);
    const availableWidth = viewportWidth - numGutters * gutter;
    if (books.length === 0 || availableWidth <= 0 || viewportHeight <= 0) {
      return { cells: [], fontSize: 1, lineHeight: 1, columns: [] };
    }
    const minL = minLines;
    const maxL = Math.max(minL + 1, Math.floor(viewportHeight / 2));
    function trySize(L) {
      const lineHeight = viewportHeight / L;
      const size = lineHeight / lineHeightRatio;
      const font = `${size}px ${fontFamily}`;
      const prepareds = books.map((b) => prepareWithSegments(b.body, font));
      const widths = prepareds.map((p) => minWidthForLineCount(p, L, availableWidth, lineHeight));
      const sum = widths.reduce((a, b) => a + b, 0);
      return { lineHeight, size, font, prepareds, widths, sum };
    }
    let lo = minL;
    let hi = maxL;
    const hiResult = trySize(hi);
    if (hiResult.sum > availableWidth) {
      lo = hi;
    } else {
      while (lo < hi) {
        const mid = lo + hi >> 1;
        if (trySize(mid).sum <= availableWidth) hi = mid;
        else lo = mid + 1;
      }
    }
    const best = lo === hi && lo === maxL ? hiResult : trySize(lo);
    const inkSum = best.widths.reduce((a, b) => a + b, 0) || 1;
    let finalWidths;
    const leftover = availableWidth - best.sum;
    if (leftover >= 0) {
      finalWidths = best.widths.map((w) => w + leftover * (w / inkSum));
    } else {
  ...
  function renderGrid(ctx2, cells, opts = {}) {
    ...
    const minOpacity = opts.minOpacity ?? 0.02;
    const width = opts.width ?? ctx2.canvas.width;
    const height = opts.height ?? ctx2.canvas.height;
    const lineHeight = opts.lineHeight ?? null;
    const fontMatch = /^(\d+(?:\.\d+)?)px\s+(.+)$/.exec(font);
    const basePx = fontMatch ? parseFloat(fontMatch[1]) : 13;
    const family = fontMatch ? fontMatch[2] : font;
    const bgHeight = lineHeight != null ? lineHeight * 0.95 : basePx * 1.1;
    ctx2.save();
    ...
    if (hasAnyBg) {
      ...
      for (let i = 0; i < cells.length; i++) {
        ...
        const scale = c.fontScale ?? 1;
        if (scale !== 1 || c.fontWeight) {
          ctx2.font = `${c.fontWeight ?? ""} ${basePx * scale}px ${family}`.trim();
        }
        const w = ctx2.measureText(c.char).width + 4;
        const h = bgHeight * scale;
        ctx2.fillStyle = c.bgColor;
        ctx2.fillRect(c.x - w / 2, c.y - h * 0.78, w, h);
        if (scale !== 1 || c.fontWeight) ctx2.font = font;
      }
    }
    ctx2.fillStyle = color;
    let prevAlpha = -1;
    let prevFontCustom = false;
    for (let i = 0; i < cells.length; i++) {
      const c = cells[i];
      const a = c.opacity ?? 1;
      ...
      if (c.color && c.color !== ctx2.fillStyle) {
        ctx2.fillStyle = c.color;
      }
      const scale = c.fontScale ?? 1;
      const weight = c.fontWeight;
      if (scale !== 1 || weight) {
        ctx2.font = `${weight ?? ""} ${basePx * scale}px ${family}`.trim();
        prevFontCustom = true;
      } else if (prevFontCustom) {
        ctx2.font = font;
        prevFontCustom = false;
      }
      ctx2.fillText(c.char, c.x, c.y);
    }
    ctx2.restore();
  }
  ...
  function sampleImage(image, sampleW, sampleH, opts = {}) {
    ...
    const h = Math.max(1, Math.floor(sampleH));
    ...
    if (fit === "contain") {
      const v = Math.round(255 * Math.min(Math.max(letterboxLuminance, 0), 1));
      ...
      ctx2.drawImage(image, 0, 0, iw, ih, dx, dy, dw, dh);
    } else {
    ...
    for (let i = 0, j = 0; i < data.length; i += 4, j++) {
      lum[j] = (0.299 * data[i] + 0.587 * data[i + 1] + 0.114 * data[i + 2]) / 255;
    }
    return { data: lum, width: w, height: h };
  }
  function sampleImageRGB(image, sampleW, sampleH, opts = {}) {
    const { fit = "cover", letterboxLuminance = 0 } = opts;
    const w = Math.max(1, Math.floor(sampleW));
    const h = Math.max(1, Math.floor(sampleH));
    const off = typeof OffscreenCanvas !== "undefined" ? new OffscreenCanvas(w, h) : Object.assign(document.createElement("canvas"), { width: w, height: h });
  ...
  function rgbAt(map, u, v) {
    const { r, g, b, width, height } = map;
  ...
  function applyBrightnessMap(cells, image, opts) {
    const {
      ...
      multiply = false,
      fit = "cover",
      letterboxLuminance = 0
    } = opts;
    const longSide = Math.max(canvasWidth, canvasHeight);
    const scale = sampleResolution / longSide;
    const sampleW = Math.max(8, Math.round(canvasWidth * scale));
    const sampleH = Math.max(8, Math.round(canvasHeight * scale));
    const lumMap = sampleImage(image, sampleW, sampleH, { fit, letterboxLuminance });
    const out = new Array(cells.length);
    for (let i = 0; i < cells.length; i++) {
      const c = cells[i];
      const u = c.x / canvasWidth;
      const v = c.y / canvasHeight;
      let l = luminanceAt(lumMap, u, v);
      if (invert) l = 1 - l;
      l = (l - 0.5) * contrast + 0.5;
      l = Math.pow(Math.max(0, Math.min(1, l)), gamma);
      let opacity = minOpacity + l * (maxOpacity - minOpacity);
      if (multiply) opacity *= c.opacity ?? 1;
      out[i] = { ...c, opacity };
    }
    return out;
  }
  ...
  function applyColorMap(cells, image, opts) {
    ...
    const sampleW = Math.max(8, Math.round(canvasWidth * scale));
    const sampleH = Math.max(8, Math.round(canvasHeight * scale));
    const rgbMap = sampleImageRGB(image, sampleW, sampleH, { fit, letterboxLuminance });
    ...
    for (let i = 0; i < cells.length; i++) {
      ...
      const u = c.x / canvasWidth;
      const v = c.y / canvasHeight;
      let { r, g, b } = rgbAt(rgbMap, u, v);
      if (sat !== 1) {
        const y = 0.299 * r + 0.587 * g + 0.114 * b;
        r = Math.max(0, Math.min(255, Math.round(y + (r - y) * sat)));
        g = Math.max(0, Math.min(255, Math.round(y + (g - y) * sat)));
        b = Math.max(0, Math.min(255, Math.round(y + (b - y) * sat)));
      }
      out[i] = { ...c, color: `rgb(${r},${g},${b})` };
    }
    return out;
  }
  ...
  function buildTokens(cells) {
    const tokens = [];
    const wordToTokens = /* @__PURE__ */ new Map();
    let current = null;
    let currentRow = -1;
    let prevCol = -2;
    const flush = () => {
      ...
      tokens.push({ word: current.word, lower, cellIndices: current.cellIndices });
      const arr = wordToTokens.get(lower);
      if (arr) arr.push(idx);
      else wordToTokens.set(lower, [idx]);
      current = null;
    };
    for (let i = 0; i < cells.length; i++) {
      const c = cells[i];
      if (c.row !== currentRow || c.col !== prevCol + 1) {
        flush();
        currentRow = c.row;
      }
      prevCol = c.col;
      if (WORD_CHAR.test(c.char)) {
        if (!current) current = { word: c.char, cellIndices: [i] };
        else {
          current.word += c.char;
          current.cellIndices.push(i);
        }
      } else {
        flush();
      }
  ...
  function wrapToLines(ctx2, words2, maxWidth) {
    ...
    for (const word of words2) {
      ...
      if (!line || ctx2.measureText(candidate).width <= maxWidth) {
        line = candidate;
      } else {
        lines.push(line);
        line = word;
  ...
  function drawStaticCitations(ctx2, opts) {
    const {
      text,
      width,
      height,
    ...
    const prefix = m ? m[1] : "";
    let px = m ? parseFloat(m[2]) : 20;
    const family = m ? m[3] : font;
    const words2 = clean.split(" ");
    const maxWidth = Math.max(1, width - padX * 2);
    let lines;
    for (; ; ) {
      ctx2.font = `${prefix}${px}px ${family}`;
      lines = wrapToLines(ctx2, words2, maxWidth);
      if (lines.length <= maxLines || px <= minFontPx) break;
      px -= 1;
    }
    const lineH = px * 1.25;
    ...
    const cx = width / 2;
    let cy = (height - blockH) / 2 + lineH / 2;
    for (const ln of lines) {
      ctx2.fillText(ln, cx, cy);
      cy += lineH;
    }
    ctx2.restore();
  }
  ...
  function words(s) {
    ...
    let m;
    while ((m = WORD_RE2.exec(s)) !== null) out.push(m[0].toLowerCase());
    return out;
  }
  function findRun(hay, needle) {
  ...
  function matchCitation(hay, citation) {
    const ranges = [];
    const segments = citation.split(/\.\.\.|…/).map((s) => s.trim()).filter(Boolean);
    for (const seg of segments) {
      const w = words(seg);
      if (!w.length) continue;
      const r = findRun(hay, w);
      if (r) {
        ranges.push(r);
        continue;
      }
      ...
      for (const sub of subs) {
        ...
        const sr = findRun(hay, sw);
        if (sr) ranges.push(sr);
      }
    }
    return ranges;
  }
  ...
  function buildLexiconTerms(canonical, lexicon) {
    const hay = canonical.map((t) => t.lower);
    return lexicon.map((e) => {
      const ranges = [];
      for (const c of e.citations || []) {
        for (const r of matchCitation(hay, c)) ranges.push(r);
      }
      return {
        term: e.philosophicalterm,
        category: e.category,
        references: e.philosophicalreferences || "",
        commentary: e.commentary || "",
        citations: e.citations || [],
        ranges: mergeRanges(ranges),
        slug: termSlug(e.philosophicalterm)
      };
    });
  }
  ...
  var chironCtx = chironCanvas.getContext("2d");
  var statusEl = document.getElementById("status");
  var controlsEl = document.getElementById("controls");
  var toggleControlsBtn = document.getElementById("toggle-controls");
  var playBtn = document.getElementById("play-relationship");
  var pauseBtn = document.getElementById("pause-relationship");
  var prevBtn = document.getElementById("prev-term");
  var nextBtn = document.getElementById("next-term");
  var intervalInput = document.getElementById("term-interval");
  var intervalValEl = document.getElementById("term-interval-val");
  var relStatusEl = document.getElementById("relationship-status");
  var categoryTextEl = document.getElementById("category-text");
  ...
  var state = {
    ...
    termImages: {
      ...
      lru: [],
      lastShown: null,
      // last image actually rendered (hold-on-miss)
      lastShownSlug: null
    },
    cssWidth: 0,
    cssHeight: 0,
    ...
    playback: {
      ...
      canonical: [],
      gridTokens: [],
      wordToGridTokens: null,
      canonicalToGrid: null,
      // Lexicon terms (the presentation authority).
      terms: [],
      termIndex: 0,
      everStarted: false,
      playing: false,
  ...
  function resizeCanvasToWindow() {
    ...
    const chironH = Math.min(
      Math.round(totalH * 0.25),
      Math.round(state.chironFontPx * 1.25 * CHIRON_LINES + 12)
    );
    document.documentElement.style.setProperty("--cat-h", CAT_H + "px");
    document.documentElement.style.setProperty("--comment-h", COMMENT_H + "px");
    document.documentElement.style.setProperty("--chiron-h", chironH + "px");
    document.documentElement.style.setProperty("--refs-h", REFS_H + "px");
    const canvasH = Math.max(1, totalH - CAT_H - COMMENT_H - chironH - REFS_H);
    canvas.width = Math.floor(w * dpr);
    canvas.height = Math.floor(canvasH * dpr);
    ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
    state.cssWidth = w;
    state.cssHeight = canvasH;
    chironCanvas.width = Math.floor(w * dpr);
    chironCanvas.height = Math.floor(chironH * dpr);
    chironCtx.setTransform(dpr, 0, 0, dpr, 0, 0);
    state.cssChironHeight = chironH;
  }
  function fontString(sizePx) {
    return `${sizePx}px ${state.fontFamily}`;
  }
  function chironFont() {
    return `500 ${state.chironFontPx}px ${state.fontFamily}`;
  }
  function canonicalSourceText() {
    if (state.books) return state.books.map((b) => b.body).join("\n\n");
    return state.text || "";
  }
  function buildCanonicalToGrid(canonical, gridTokens) {
    const map = new Int32Array(canonical.length).fill(-1);
    let g = 0;
    for (let c = 0; c < canonical.length && g < gridTokens.length; c++) {
      while (g < gridTokens.length && gridTokens[g].lower !== canonical[c].lower) g++;
      if (g < gridTokens.length) {
        map[c] = g;
        g++;
      }
    }
  ...
  function rebuildPlaybackTokens() {
    const canonical = buildRawTokens(canonicalSourceText());
    const built = buildTokens(state.baseCells);
    const pb = state.playback;
    pb.canonical = canonical;
    pb.gridTokens = built.tokens;
    pb.wordToGridTokens = built.wordToTokens;
    pb.canonicalToGrid = buildCanonicalToGrid(canonical, built.tokens);
    if (state.lexicon.length) {
      pb.terms = buildLexiconTerms(canonical, state.lexicon);
  ...
  function prettifyCategory(cat) {
    return (cat || "").replace(/\band\b/g, "&");
  }
  function citationTextFor(term) {
    if (!term) return "";
    if (term.citations && term.citations.length) return term.citations.join("  \u2022  ");
    return term.term;
  }
  function rebuildGrid() {
    const t0 = performance.now();
    if (!state.books) return;
    const result = buildColumnsGrid(
      state.books,
      ctx,
      state.fontFamily,
      state.lineHeightRatio,
      state.cssWidth,
      state.cssHeight,
      { gutter: state.gutter }
    );
  ...
  function effectiveImageForTerm(term) {
    let img = state.image;
    if (state.options.perTermImages && state.playback.everStarted && term) {
      const slug = term.slug;
      const tImg = getTermImage(slug);
      if (tImg) {
        state.termImages.lastShown = tImg;
        state.termImages.lastShownSlug = slug;
      }
      if (state.termImages.lastShown) {
        img = state.termImages.lastShown;
        if (state.options.termWatermark && state.termImages.lastShownSlug) {
          const entry = state.termImages.cache.get(state.termImages.lastShownSlug);
          if (entry && entry.img) {
            if (!entry.composite) {
              const t = state.playback.terms.find((x) => x.slug === state.termImages.lastShownSlug);
              entry.composite = buildTermComposite(t ? t.term : "", entry.img);
            }
            img = entry.composite;
          }
        }
  ...
  function renderCanvas() {
    ...
    if (effectiveImage) {
      if (state.options.colorMap) {
        cells = applyColorMap(cells, effectiveImage, {
          canvasWidth: state.cssWidth,
          canvasHeight: state.cssHeight,
          saturation: state.options.saturation,
          fit: state.options.fit,
          letterboxLuminance: state.options.letterboxLuminance
        });
      } else {
        cells = applyBrightnessMap(cells, effectiveImage, {
          canvasWidth: state.cssWidth,
          canvasHeight: state.cssHeight,
          gamma: state.options.gamma,
          contrast: state.options.contrast,
          invert: state.options.invert,
          minOpacity: state.options.minOpacity,
          fit: state.options.fit,
          letterboxLuminance: state.options.letterboxLuminance
        });
      }
  ...
  function renderChiron() {
    const light = state.options.lightBg;
    drawStaticCitations(chironCtx, {
      text: state.playback.everStarted ? citationTextFor(currentTerm()) : "",
      width: state.cssWidth,
      height: state.cssChironHeight,
      font: chironFont(),
      color: light ? "#111" : "#f4f4f4",
      bg: light ? "#fff" : "#000",
      maxLines: CHIRON_LINES
    });
  }
  function updateTermUI() {
    const term = currentTerm();
    if (!term) {
      categoryTextEl.innerHTML = "";
      commentaryTextEl.textContent = "";
      referencesTextEl.textContent = "";
      relStatusEl.textContent = "";
      return;
    }
    categoryTextEl.innerHTML = `<span class="cat">${escapeHtml(prettifyCategory(term.category))}</span> \u2014 <span class="term">${escapeHtml(term.term)}</span>`;
    commentaryTextEl.textContent = term.commentary || "";
    referencesTextEl.textContent = term.references || "";
    const pb = state.playback;
    relStatusEl.textContent = `${pb.playing ? "\u25B6" : "\u275A\u275A"} term ${pb.termIndex + 1}/${pb.terms.length} \xB7 ${term.ranges.length} passage${term.ranges.length === 1 ? "" : "s"}`;
  }
  function escapeHtml(s) {
    return String(s).replace(/[&<>"]/g, (c) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;" })[c]);
  }
  function goToTerm(i, { manual = false } = {}) {
    const pb = state.playback;
    if (!pb.terms.length) return;
    const n = pb.terms.length;
    pb.termIndex = (i % n + n) % n;
    pb.everStarted = true;
    updateTermUI();
    onTermChanged();
  ...
  function bumpIdle() {
    clearIdle();
    state.playback.idleTimerId = setTimeout(() => {
      state.playback.idleTimerId = null;
      startPlaying();
    }, IDLE_RESUME_MS);
  }
  function clearIdle() {
    const pb = state.playback;
    if (pb.idleTimerId != null) {
      clearTimeout(pb.idleTimerId);
      pb.idleTimerId = null;
    }
  }
  var LRU_CAP = 80;
  var PREFETCH_AHEAD = 4;
  function buildTermComposite(term, img) {
    ...
    const text = (term || "").toUpperCase();
    const fontStack = 'Helvetica, Arial, "Helvetica Neue", sans-serif';
    let fontPx = Math.max(12, Math.floor(h * 0.16));
    cx.font = `900 ${fontPx}px ${fontStack}`;
    const targetW = w * 0.8;
    const measured = cx.measureText(text).width;
    if (measured > 0) {
      fontPx = Math.max(12, Math.floor(fontPx * (targetW / measured)));
      cx.font = `900 ${fontPx}px ${fontStack}`;
    }
    cx.textAlign = "center";
    cx.textBaseline = "middle";
    cx.shadowColor = "rgba(0,0,0,0.6)";
    cx.shadowBlur = Math.max(8, Math.round(fontPx * 0.25));
    cx.shadowOffsetX = 0;
    cx.shadowOffsetY = 0;
    cx.fillStyle = "rgba(255,230,0,0.7)";
  ...
  function loadManifest() {
    ...
    if (ti.manifest || ti.manifestLoading) return ti.manifestLoading || Promise.resolve(ti.manifest);
    if (window.__DIST__) {
      ti.manifest = window.__DIST__.manifest || {};
      return Promise.resolve(ti.manifest);
    }
    ti.manifestLoading = fetch("./images/terms/manifest.json", { cache: "no-cache" }).then((r) => {
      if (!r.ok) throw new Error(`HTTP ${r.status}`);
      return r.json();
    }).then((json) => {
      ti.manifest = json || {};
      ti.manifestLoading = null;
      return ti.manifest;
    }).catch((err) => {
      ti.manifest = {};
      ti.manifestLoading = null;
      setStatus(`manifest unavailable (${err.message})`);
      return ti.manifest;
    });
  ...
  function bumpLRU(slug) {
    const ti = state.termImages;
    const i = ti.lru.indexOf(slug);
    if (i >= 0) ti.lru.splice(i, 1);
    ti.lru.push(slug);
    while (ti.lru.length > LRU_CAP) {
      const evict = ti.lru.shift();
      if (evict) ti.cache.delete(evict);
    }
  }
  function getTermImage(slug) {
    const entry = state.termImages.cache.get(slug);
    return entry && entry.img ? entry.img : null;
  }
  function ensureTermImage(slug) {
    const ti = state.termImages;
    if (!slug) return null;
    let entry = ti.cache.get(slug);
    if (entry && entry.img) {
      bumpLRU(slug);
      return entry.img;
    }
    ...
    bumpLRU(slug);
    entry.loading = loadImagePromise(url).then((img) => {
      if (ti.cache.get(slug) === entry) {
        entry.img = img;
        entry.loading = null;
        if (state.options.perTermImages) renderCanvas();
      }
    }).catch(() => {
      if (ti.cache.get(slug) === entry) entry.loading = null;
    });
    return null;
  }
  function prefetchAroundTerm() {
    const pb = state.playback;
    if (!pb.terms.length) return;
    for (let k = 0; k <= PREFETCH_AHEAD; k++) {
      const t = pb.terms[(pb.termIndex + k) % pb.terms.length];
      if (t) ensureTermImage(t.slug);
    }
  }
  function onTermChanged() {
    if (!state.options.perTermImages) return;
    const t = currentTerm();
    if (t) ensureTermImage(t.slug);
    prefetchAroundTerm();
  }
  intervalInput.addEventListener("input", () => {
    state.playback.intervalMs = Math.round(parseFloat(intervalInput.value) * 1e3);
    intervalValEl.textContent = intervalInput.value;
    if (state.playback.playing) scheduleTick();
  });
  playBtn.addEventListener("click", () => {
    if (state.playback.terms.length) startPlaying();
  });
  ...
  prevBtn.addEventListener("click", () => {
    stopPlaying();
    goToTerm(state.playback.termIndex - 1, { manual: true });
  });
  ...
  function setControlsCollapsed(collapsed) {
    ...
    toggleControlsBtn.textContent = collapsed ? "\u25B8" : "Hide \u25BE";
    toggleControlsBtn.title = collapsed ? "Show controls (h)" : "Hide controls (h)";
  }
  ...
  window.addEventListener("keydown", (e) => {
    ...
    if (e.key === " " || e.code === "Space") {
      if (!state.playback.terms.length) return;
      e.preventDefault();
  ...
  async function boot() {
    ...
    try {
      if (window.__DIST__) {
        state.text = window.__DIST__.text;
        state.lexicon = window.__DIST__.lexicon;
      } else {
        const [srcRes, lexRes] = await Promise.all([
          fetch("./text/tenderbuttons.txt", { cache: "no-cache" }),
          fetch("./text/tender_buttons_lexicon.json", { cache: "no-cache" })
        ]);
        if (!srcRes.ok) throw new Error(`source HTTP ${srcRes.status}`);
        if (!lexRes.ok) throw new Error(`lexicon HTTP ${lexRes.status}`);
        state.text = await srcRes.text();
        state.lexicon = await lexRes.json();
      }
      state.books = splitTenderButtons(state.text);
    } catch (err) {
    ...
    rebuildGrid();
    renderCanvas();
    if (state.options.perTermImages) {
      await loadManifest();
    }
    applyUrlParams();
    if (!location.search.includes("play=0")) {
      startPlaying();
    } else {
      goToTerm(state.playback.termIndex);
      stopPlaying();
    }
  }
  function applyUrlParams() {
    ...
    if (termParam != null && state.playback.terms.length) {
      const n = parseInt(termParam, 10);
      if (Number.isFinite(n)) {
        const clamped = Math.max(0, Math.min(n, state.playback.terms.length - 1));
        state.playback.termIndex = clamped;
        if (clamped !== n) setStatus(`?term=${n} clamped to ${clamped}`);
      }
    }
    const intervalParam = params.get("interval");
    if (intervalParam != null) {
      const s = parseFloat(intervalParam);
      if (Number.isFinite(s) && s >= 1) {
        state.playback.intervalMs = Math.round(s * 1e3);
        intervalInput.value = String(s);
        intervalValEl.textContent = String(s);
      }
    }
  }
  boot();
